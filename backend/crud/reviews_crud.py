...
async def create_review(product_id: int, user_id: int, rating: int, comment: str):
    query = """
    INSERT INTO reviews(product_id, user_id, rating, comment)
    VALUES (:product_id, :user_id, :rating, :comment)
    """
    try:
        review_id = await database.execute(query=query, values={
...
async def update_review(rating: int, comment: str, review_id: int):
    query = """
    UPDATE reviews SET rating = :rating, comment = :comment
    WHERE review_id = :review_id
    """
    try:
        await database.execute(query=query, values={"rating": rating, "comment": comment, "review_id": review_id})
        return True
    except:
